...
class LV_ViT(nn.Module):
    ...
    def forward(self, pixel_values):
        x = self.forward_embeddings(pixel_values)
...
@register_model
def lvvit_t(pretrained=False, pretrained_cfg_overlay=None, **kwargs):
    model_args = dict(patch_size=16, embed_dim=240, depth=12, num_heads=4, mlp_ratio=3.,
...
@register_model
def lvvit_s(pretrained=False, pretrained_cfg_overlay=None, **kwargs):
    model_args = dict(patch_size=16, embed_dim=384, depth=16, num_heads=6, mlp_ratio=3.,
...
@register_model
def lvvit_m(pretrained=False, pretrained_cfg_overlay=None, **kwargs):
    model_args = dict(patch_size=16, embed_dim=512, depth=20, num_heads=8, mlp_ratio=3.,
                   p_emb='4_2', skip_lam=2., return_dense=True, mix_token=True)
    model = build_model_with_cfg(
        LV_ViT,
        "lvvit_m",
...
@register_model
def lvvit_l(pretrained=False, pretrained_cfg_overlay=None, **kwargs):
    order = ['tr'] * 24  # this will override depth, can also be set as None
    model_args = dict(patch_size=16, embed_dim=768, depth=24, num_heads=12, mlp_ratio=3.,
